...
const evenFinder = function(nums){
  let evens = []
  let odds = []
...
const divider = (numbersArray) => [evenFinder(numbersArray),oddFinder(numbersArray)]
...
function finder(arr){
  ...
  let randomLoc = arr.indexOf(randomNum)
  if(randomLoc > 0){
    return true
  } else{
    return false;
...
function removeItem(groceryList,item){
	let location = groceryList.indexOf(item);
	if(location){
...
var user2 = {
  name: "Sam O",
  email: "sam.o@dev.mountain",
  password: "lastTangoinParis",
  username: "wholetthedogsout"
}

var user3 = {
  name: "Tiny O",
  email: "tiny.o@dev.mountain",
  password: "lastTangoinSaoTome",
  username: "wholetthecatsout"
}
